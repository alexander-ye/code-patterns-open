...
class Solution:
  def twoSum(self, nums: List[int], target: int) -> List[int]:
    ...
    for i in range(N):
      ...
      number = nums[i]
      # Compute the number's complement
...
class Solution {
    public int[] twoSum(int[] nums, int target) {
        // Declare a hash map to store our solution
        Map<Integer, Integer> complements = new HashMap<>();

        // For each index of our nums list
        for (int i = 0; i < nums.length; i++) {
            // Get the number at that index
            int value = nums[i];
            // Compute the number's complement
            ...
            if (complements.containsKey(difference)) {
                // If the complement is in the hash map, we return the index of our two numbers
                int[] out = new int[]{i, complements.get(difference)};
                return out;
            } else {
                // If not, add our current number to the hashmap
                complements.put(value, i);
            }
        }
        // Java requires this return statement, even though
        return new int[]{};
    }
}
```
...
var twoSum = function (nums, target) {
  ...
  for (let i = 0; i < nums.length; i++) {
    const num = nums[i];
    const leftover = target - num;
    if (leftover in leftovers) {
      return [i, leftovers[leftover]];
    } else {
      leftovers[num] = i;
    }
  }
